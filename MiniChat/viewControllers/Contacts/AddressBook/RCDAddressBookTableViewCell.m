...
- (void)initSubviews {
    ...
    [_acceptBtn addTarget:self action:@selector(acceptButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)acceptButtonClick {
    if (self.acceptButtonClickBlock) {
        self.acceptButtonClickBlock(_selectedIndexPath);
    }
}
...
- (void)setModel:(RCUserInfo *)user {
    if (user) {
        ...
        if ([user.portraitUri isEqualToString:@""]) {
            ...
            [self.portraitImageView chm_imageViewWithURL:user.portraitUri placeholder:@"icon_person"];
